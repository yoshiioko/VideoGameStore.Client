...
public class NotAuthenticatedClient
{
    ...
    public NotAuthenticatedClient(HttpClient httpClient)
    {
    ...
    public async Task<PagedResponse<Game>> GetGamesAsync(int pageNumber, string? filter)
    {
        ...
        var pagedResponse = new PagedResponse<Game>
        {
            ...
            PagingInfo = paginationHeader is not null ? JsonSerializer.Deserialize<PagingInfo>(paginationHeader, options) ?? new PagingInfo() : new PagingInfo()
        };
